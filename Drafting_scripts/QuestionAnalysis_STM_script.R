...
toppingtopicswords %>%
  filter(topic %in% c(54, 47, 40, 65, 63)) %>% 
...
toppingtopicswords %>%
  filter(topic %in% c(66, 34, 75, 73, 2)) %>% 
  ggplot(aes(term, beta, fill = topic)) + 
  geom_bar(stat = "identity") + 
  facet_wrap( ~ topic, 
              ncol = 3, 
...
secondtopicswords %>%
  filter(topic %in% c(54, 55, 47, 73, 51)) %>% 
  ggplot(aes(term, beta, fill = topic)) + 
  geom_bar(stat = "identity") + 
  facet_wrap( ~ topic, 
              ncol = 3, 
...
secondtopicswords %>%
  filter(topic %in% c(67, 27, 39, 26, 38)) %>% 
  ggplot(aes(term, beta, fill = topic)) + 
  geom_bar(stat = "identity") + 
  facet_wrap( ~ topic, 
              ncol = 3, 
...
thirdtopicswords %>%
  filter(topic %in% c(55, 38, 54, 20, 30)) %>% 
  ggplot(aes(term, beta, fill = topic)) + 
  geom_bar(stat = "identity") + 
  facet_wrap( ~ topic, 
              ncol = 3, 
...
# Topic 21 is about generic higher education 
...
#Topic 42 is about sustainability. 
...
#### Lets instead look at the topics for education, and how they 
...
sum(str_detect(questiontextwparty$text_a_l, "bærekraftsmål 4"))
...
sum(str_detect(questiontextwparty$justification_l, "utdanning for bærekraftig utvikling"))
sum(str_detect(questiontextwparty$justification_l, " ubu "))

sum(str_detect(questiontextwparty$text_a_l, "delmål 4.7"))
sum(str_detect(questiontextwparty$text_a_l, "utdanning for bærekraftig utvikling"))
sum(str_detect(questiontextwparty$text_a_l, " ubu "))

sum(str_detect(questiontextwparty$text_q_l, "delmål 4.7"))
sum(str_detect(questiontextwparty$text_q_l, "utdanning for bærekraftig utvikling"))
sum(str_detect(questiontextwparty$text_q_l, " ubu "))
